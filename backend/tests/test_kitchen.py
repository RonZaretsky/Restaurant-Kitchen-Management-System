...
async def test_a_served_orders_ready_item_is_excluded(client: AsyncClient, db_session: AsyncSession) -> None:
    # Arrange: a ready item whose Order is then marked served through the real serve
    # endpoint, not a direct DB write. A served Order's items keep their own ready
    # status, and would otherwise leak onto this board forever.
