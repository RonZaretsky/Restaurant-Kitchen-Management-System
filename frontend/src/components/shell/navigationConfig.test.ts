...
// Focused unit coverage of canRoleVisit's includeSubroutes clause, on top of router.test.tsx's
// own integration-level coverage of the same behavior through the real route tree. Exercised
// directly here since router.test.tsx cannot isolate the nav clause from the prefix clause for
// every case: most nav entries live under their own owner's prefix, so only Admin's
// cross-prefix Ingredients entry can prove the nav clause's own behavior in isolation.
