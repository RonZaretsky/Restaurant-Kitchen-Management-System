...
FR-3: An Admin can create a User account, edit an existing User's Role or full name, deactivate an active User, and reactivate a previously deactivated User.
...
FR-24: An Admin can add and configure Restaurant Tables (table number, capacity); table numbers are unique.
...
UX-DR17: Implement inline (non-toast) rejection microcopy for the named validation failures: unavailable-dish add (FR-5), duplicate username/table-number/ingredient-name (FR-3/FR-24/FR-16), last-admin lockout (AD-15), invalid login credentials (generic, no enumeration).
...
_Populated in Step 2 once epics are designed._

## Epic List

### Epic 1: Staff Accounts & Access Control
Every role can log in securely and see only what their role permits; Admin can manage the staff roster.
...
FR-24: Epic 2 - Manage restaurant tables
...
**And** invalid credentials (wrong username or password) are rejected with a generic "Invalid username or password" error that does not reveal which part was wrong (FR-1)
...
**And** the empty leftover `backend/data_models/exceptions/` folder is removed (or relocated) as part of this story, not treated as a second exceptions convention alongside `backend/exceptions/` (architecture spine, Deferred)
...
**And** authorization is checked via one shared FastAPI dependency (AD-3), never re-derived per route
...
**Given** valid new-user details (username, full name, role)
**When** an Admin submits the create-user form
**Then** a new User is created and can log in immediately with the assigned Role (FR-3)
...
**And** the Users screen renders per the UX mocks with dense-row list styling (UX-DR8, UX-DR19) and meets the WCAG AA accessibility floor (UX-DR21)
...
**When** a future Order Item for that Dish is later prepared (Epic 5)
**Then** stock deduction reflects the Recipe as currently defined, not a stale copy from order time (FR-23)
**And** the availability gate control (UX-DR13) re-enables instantly, no page reload, the moment a first ingredient line is added
...
**And** the Tables setup screen renders per the UX mocks with dense-row list styling (UX-DR8, UX-DR19)
...
**And** the Tables grid shows the table-status badge for available/occupied/reserved (UX-DR2), with "No tables configured yet" shown if none exist (UX-DR15)
...
**And** the Order Item row shows its status badge (UX-DR1), with "No items added yet" shown on a fresh Order (UX-DR15)

**Given** `price_at_add` does not yet exist as a column on `OrderItem`
**When** this story adds it
**Then** Alembic is adopted for the first time (async template, per AD-4): a baseline revision is generated against the current schema, this story's own migration adds `price_at_add` on top of it, and `Base.metadata.create_all` is removed from `backend/container.py`'s startup path (AD-4)
...
**Given** no WebSocket endpoint exists yet in the codebase
**When** this story is built
**Then** a single WebSocket endpoint is adopted, one connection per authenticated session, gated by the same JWT verified for REST routes (AD-3), scoped to the connecting User's Role; the `websockets` package already present transitively via `uvicorn[standard]` backs it, no new dependency needed; every state change is emitted exactly once, by the service layer that owns the mutation, under the `{domain}.{event}` naming convention (past-tense, e.g. `order.item_status_changed`), fixed here so no later story reinvents the naming (AD-2)

**Given** any Order Item's status changes (from Epic 5's Cook actions)
**When** the change is committed
**Then** it appears on the Waiter's Tables grid and Table/Order detail within 2 seconds via WebSocket push, with no manual refresh (FR-6, NFR-1, AD-2)
...
**And** a cancelled Order Item is excluded from Epic 5's status-derivation (FR-12) and readiness-for-close checks (FR-8); NFR-6 last-write-wins governs concurrent field edits outside the atomic transition paths

**Given** `cancelled` does not yet exist as a value on the `OrderItemStatus` enum
**When** this story adds it
**Then** the enum change ships with its own Alembic migration on top of Story 3.2's baseline, per AD-4 (exact literal name left to this migration, per the architecture spine's Deferred section)
...
**And** the movement type chip renders with a neutral color scheme, distinct from the status traffic-light convention (UX-DR14)
...
**And** the Alerts nav badge shows a persistent count with no toast (UX-DR5), the Alerts screen lists one Alert row per Ingredient-in-shortage reading "Stock low: {ingredient} ({current stock}{unit} left)" (UX-DR10), and "No active shortages" is shown when the list is empty (UX-DR15)
...
**And** the Kitchen Display defaults to dark theme (UX-DR7), cards render at elevation 1, and status badges show on each Order Item row within a card (UX-DR1)
...
**And** the Kitchen Display card's advance-status control is a single large click target (UX-DR19), and the status badge updates instantly on both the Kitchen Display and the Waiter's screen via the same WebSocket push (AD-2)
...
**And** the Close action has no separate confirm step, and the Waiter's "tables need attention" counter clears for that table the moment it's marked served (UX-DR4)
...
**And** the OpenAI call goes through a `backend/clients/` adapter behind an interface, never called directly from `services/` (AD-12), and the Recipe Suggestion card shows the requesting Cook and the ingredients drawn on (UX-DR11)
...
**And** the Recipe Suggestion card's two actions are Confirm into Dish (primary button, UX-DR6) and Dismiss (outlined/text button, UX-DR11), with "No suggestions awaiting review" shown when the list is empty (UX-DR15)

**Given** `dismissed` does not yet exist as a column on `AIRecipeSuggestion`
**When** this story adds it
**Then** it ships with its own Alembic migration on top of the baseline established in Story 3.2, per AD-4
...
**And** "No chat sessions yet" is shown when none exist (UX-DR15)
