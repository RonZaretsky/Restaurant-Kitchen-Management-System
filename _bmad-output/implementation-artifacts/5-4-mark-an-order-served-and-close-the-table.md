...
| 2026-08-21 | Manual testing found a real bug: `TableOrderDetailPage.tsx`'s "no open order" banner and the just-closed Order's stale item table/total bar rendered simultaneously after Close (the `order` object from the last successful fetch stays truthy even once the query 404s), persisting until a manual reload. Fixed by gating the order-content block on `!hasNoOpenOrder` too, making the two states mutually exclusive. Also added navigation back to `/waiter/tables` on a successful Close (previously stayed on the now-nonexistent Order's page), matching `TablesPage.tsx`'s own open-then-navigate precedent. One frontend test rewritten to assert the navigation, one new test added asserting the two states never co-render. `npx tsc -b` clean, `TableOrderDetailPage.test.tsx` 31/31 passed. |
| 2026-08-21 | Follow-up manual test found the banner could still flash briefly: `useCloseOrder`'s own `onSettled` invalidated the closed Order's query key, and on localhost that background refetch could resolve (and re-render `hasNoOpenOrder: true`) before the navigation's route change fully committed. Fixed by no longer invalidating `orderForTableQueryKey` in `useCloseOrder` at all — the caller navigates away on success regardless, so there is nothing left on this page that needs a fresh Order to render. `useCloseOrder` dropped its now-unused `tableId` parameter. `npx tsc -b` clean, `TableOrderDetailPage.test.tsx` 31/31 passed. |
